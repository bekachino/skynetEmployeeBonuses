  ...
  return (
    <div className="date-picker">
      <div className="date-picker-input" onClick={(e) => e.stopPropagation()}>
        ...
        <div
          className={`date-picker-panel${popupId === `date-picker${i || ''}` ? '-open' : ''}`}
        >
          ...
          <div className="day-picker-month-days">
            {calendar.map((day, index) => {
              ...
              return (
                <div
                  className={`date-picker-month-day ${
                    day.format('MMMM') !== currentMonthName
                      ? 'date-picker-not-current-month'
                      : ''
                  } ${
                    day.isSame(value, 'day') ? 'date-picker-current-day' : ''
                  } ${
                    day.isAfter(moment(), 'day') || day.isSame(moment(), 'day')
                      ? 'date-picker-day-in-future'
                      : ''
                  } ${isWeekend ? 'day-picker-day-weekend' : ''}`}
                  key={index}
                  onClick={() => {
                    if (
        ...
        <span className="date-picker-toggler" onClick={onCheckboxClick} />
